...
&emsp;&emsp;大多情况下，数据结构和算法都未能有一套标准,导致被迫从事大量重复工作。

&emsp;&emsp;为了建立数据结构和算法的一套标准,诞生了**STL**。

&emsp;&emsp;STL(Standard Template Library,**标准模板库**)

&emsp;&emsp;STL 从广义上分为: **容器(container) 算法(algorithm) 迭代器(iterator)**。

&emsp;&emsp;**容器**和**算法**之间通过**迭代器**进行无缝连接。

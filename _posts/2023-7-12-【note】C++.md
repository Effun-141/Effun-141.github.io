...
*[翁凯老师C++课程笔记](https://www.bilibili.com/video/BV1dE41167hJ/)*（每一小节的序号略有调整）和 *[黑马程序员C++课程笔记](https://www.bilibili.com/video/BV1et411b73Z/)*（泛型编程部分主要记录黑马程序员课程）。
...
<figure>
    <img src="https://effun.xyz/images/C-plus-plus/转换操作符.png" width=400px>
    <center>
    <figcaption>转换操作符</figcaption>
    </center>
</figure>

&emsp;&emsp;没有返回类型，因为`double()`已经可以知道它的返回类型了。
...
&emsp;&emsp;T代表操作符名字，可以是任意类型。

&emsp;&emsp;没有参数，没有返回类型，编译器会用这个操作符将X转换成T。

&emsp;&emsp;两种类型转换方法：

<figure>
    <img src="https://effun.xyz/images/C-plus-plus/两种方法.png" width=400px>
    <center>
    <figcaption>两种类型转换方法</figcaption>
    </center>
</figure>

&emsp;&emsp;尽量不要使用这种自动的方式，虽然可以用explicit控制。还是自己写一个比较好。

# 8 模板——template

&emsp;&emsp;模板允许我们重用代码，类型成为了参数。Perform similar operations on different types of data.
...
&emsp;&emsp;template是declaration而不是definition。

&emsp;&emsp;模板必须要确定出T的数据类型，才可以使用
...
&emsp;&emsp;类模板里面的每一个函数都是函数模板，并且在这些函数中间类的名字后面也要加上<>。

&emsp;&emsp;Example: Vector
...
&emsp;&emsp;模板可以有多个参数。可以用单个大写字母避免误解。
...
&emsp;&emsp;模板参数可以是常量形式，如果不指定常量的大小，则使用默认的大小。
...
<figure>
    <img src="https://effun.xyz/images/C-plus-plus/三种情况.png" width=400px>
    <center>
    <figcaption>模板与继承的三种情况</figcaption>
    </center>
</figure>
...
&emsp;&emsp;模板就是建立**通用的模具**，大大**提高复用性**。

&emsp;&emsp;模板不可以直接使用，只是一个通用框架。

### 9.1.2 函数模板

&emsp;&emsp;C++提供两种模板机制:**函数模板**和**类模板** 。

&emsp;&emsp;函数模板作用：建立一个通用函数，其函数返回值类型和形参类型可以不具体制定，用一个**虚拟的类型**来代表。
...
&emsp;&emsp;**使用模板时必须确定出通用数据类型T，并且能够推导出一致的类型。**

&emsp;&emsp;**普通函数与函数模板区别：**

* &emsp;&emsp;普通函数调用时可以发生自动类型转换（隐式类型转换）。
* &emsp;&emsp;函数模板调用时，如果利用自动类型推导，不会发生隐式类型转换。
* &emsp;&emsp;如果利用显示指定类型的方式，可以发生隐式类型转换。

&emsp;&emsp;建议使用显示指定类型的方式，调用函数模板，因为可以自己确定通用类型T。

&emsp;&emsp;普通函数和函数模板可以发生重载，调用规则如下：

&emsp;&emsp;1. 如果函数模板和普通函数都可以实现，优先调用普通函数。

&emsp;&emsp;2. 可以通过空模板参数列表来强制调用函数模板。

&emsp;&emsp;3. 函数模板也可以发生函数重载。

&emsp;&emsp;4. 如果函数模板可以产生更好的匹配,优先调用函数模板。
...
&emsp;&emsp;**提供了函数模板，最好就不要提供普通函数，否则容易出现二义性！**

&emsp;&emsp;模板也具有一定的局限性。
...
* &emsp;&emsp;利用具体化的模板，可以解决自定义类型的通用化
* &emsp;&emsp;学习模板并不是为了写模板，而是在STL能够运用系统提供的模板
...
&emsp;&emsp;类模板与函数模板区别主要有两点：

* &emsp;&emsp;类模板没有自动类型推导的使用方式。
* &emsp;&emsp;类模板在模板参数列表中可以有默认参数。
...
&emsp;&emsp;类模板中成员函数和普通类中成员函数创建时机是有区别的：

* &emsp;&emsp;普通类中的成员函数一开始就可以创建。
* &emsp;&emsp;类模板中的成员函数在调用时才创建。
...
&emsp;&emsp;即：类模板中的成员函数并不是一开始就创建的，在调用时才去创建。

&emsp;&emsp;类模板对象做函数参数一共有三种传入方式：

&emsp;&emsp;1. 指定传入的类型：直接显示对象的数据类型

&emsp;&emsp;2. 参数模板化：将对象中的参数变为模板进行传递

&emsp;&emsp;3. 整个类模板化：将这个对象类型 模板化进行传递
...
&emsp;&emsp;**最常用的是直接指定传入模型。**

&emsp;&emsp;当类模板碰到**继承**时，需要注意一下几点：

* &emsp;&emsp;当子类继承的父类是一个类模板时，子类在声明的时候，要指定出父类中T的类型。
* &emsp;&emsp;如果不指定，编译器无法给子类分配内存。
* &emsp;&emsp;如果想灵活指定出父类中T的类型，子类也需变为类模板。
...
&emsp;&emsp;类模板成员函数类外实现：类模板中成员函数类外实现时，需要加上模板参数列表
...
&emsp;&emsp;**类模板中成员函数创建时机是在调用阶段，导致分文件编写时链接不到！所以当分文件编写类模板文件时不能直接包含头文件。**

* &emsp;&emsp;解决方式1：直接包含.cpp源文件。
* &emsp;&emsp;解决方式2：将声明和实现写到同一个文件中，并更改后缀名为.hpp，hpp是约定的名称，并不是强制。
...
&emsp;&emsp;**全局函数类内实现**：直接在类内声明友元即可。

&emsp;&emsp;**全局函数类外实现**：需要提前让编译器知道全局函数的存在。
...
&emsp;&emsp;全局函数类外实现需要加一个空参数列表`<>`。

## 9.2 STL 容器

&emsp;&emsp;长久以来，软件界一直希望建立一种可重复利用的东西。

&emsp;&emsp;C++的**面向对象**和**泛型编程**思想，目的就是**复用性的提升**。

&emsp;&emsp;大多情况下，数据结构和算法都未能有一套标准,导致被迫从事大量重复工作。为了建立数据结构和算法的一套标准,诞生了**STL**。

&emsp;&emsp;STL(Standard Template Library,**标准模板库**)。

&emsp;&emsp;STL 从广义上分为: **容器(container) 算法(algorithm) 迭代器(iterator)**。**容器**和**算法**之间通过**迭代器**进行无缝连接。

&emsp;&emsp;STL 几乎所有的代码都采用了模板类或者模板函数。

&emsp;&emsp;STL大体分为六大组件，分别是:**容器、算法、迭代器、仿函数、适配器（配接器）、空间配置器**。
...
&emsp;&emsp;STL**容器**就是将运用**最广泛的一些数据结构**实现出来。常用的数据结构：数组, 链表,树, 栈, 队列, 集合, 映射表等。

&emsp;&emsp;这些容器分为**序列式容器**和**关联式容器**两种:

* &emsp;&emsp;序列式容器：强调值的排序，序列式容器中的每个元素均有固定的位置。
* &emsp;&emsp;关联式容器:二叉树结构，各元素之间没有严格的物理上的顺序关系。

&emsp;&emsp;**算法：**——问题之解法

&emsp;&emsp;有限的步骤，解决逻辑或数学上的问题，这一门学科我们叫做算法(Algorithms)。

&emsp;&emsp;算法分为:**质变算法**和**非质变算法**。

* 质变算法：是指运算过程中会更改区间内的元素的内容。例如拷贝，替换，删除等等。

* 非质变算法：是指运算过程中不会更改区间内的元素内容，例如查找、计数、遍历、寻找极值等等。

&emsp;&emsp;**迭代器：**——容器和算法之间粘合剂

&emsp;&emsp;提供一种方法，使之能够依序寻访某个容器所含的各个元素，而又无需暴露该容器的内部表示方式。**每个容器都有自己专属的迭代器。**

&emsp;&emsp;迭代器使用非常类似于指针，初学阶段我们可以先理解迭代器为指针。
...
&emsp;&emsp;常用的容器中迭代器种类为双向迭代器，和随机访问迭代器。

### 9.2.2 Vector容器

&emsp;&emsp;STL中最常用的容器为Vector，可以理解为数组。

&emsp;&emsp;**容器：`vector`**

&emsp;&emsp;**算法：`for_each`**

&emsp;&emsp;**迭代器：`vector<int>::iterator`**

&emsp;&emsp;使用前要包含头文件 `vector`
...
&emsp;&emsp;**功能：**vector数据结构和**数组非常相似**，也称为**单端数组**。不同之处在于数组是静态空间，而vector可以**动态扩展**。

&emsp;&emsp;**动态扩展：**并不是在原空间之后续接新空间，而是找更大的内存空间，然后将原数据拷贝新空间，释放原空间。

&emsp;&emsp;**vector构造函数**：创建vector容器

**函数原型：**
* &emsp;&emsp;`vector<T> v; `采用模板实现类实现，默认构造函数
* &emsp;&emsp;`vector(v.begin(), v.end());   `将v[begin(), end())区间中的元素拷贝给本身。
* &emsp;&emsp;`vector(n, elem);`构造函数将n个elem拷贝给本身。
* &emsp;&emsp;`vector(const vector &vec);`拷贝构造函数。

...
- [ESVIO](#ESVIO)
  - [整体逻辑](#整体逻辑)
  - [功能包1：feature_tacker(前端特征提取与匹配)](#功能包1feature_tacker前端特征提取与匹配)
    ...
    - [18.FeatureTracker::rejectWithF_event函数](#18FeatureTrackerrejectWith_Fevent函数)
  - [功能包2：esvio_estimator(位姿估计)](#功能包2esvio_estimator位姿估计)
    - [1.stereo_estimator_node的main函数](#1stereo_estimator_node的main函数)
...
该特征点基于Fast角点改进而来，使用SAE平面，以时间戳大小代替像素点强度来进行角点检测，前序文章是Scaramuzza组提出的[**eFast角点**](https://www.zora.uzh.ch/id/eprint/138925/)。核心思想是认为当某一个像素周围两个内外圆形区域上的时间戳圆弧满足特定长度要求时将其判定为Arc特征，从论文中的图中能比较直观的体会到。
...
代码中没有用到有参构造函数,使用的是无参构造函数，只初始化了常量成员变量。然后在`stereo_event_tracker_node.cpp`中，根据`Do_motion_correction`参数调用不同版本的`init`函数完成完整初始化：
...
包含两个节点：stereo_esvio_estimator 和 stereo_esio_estimator节点，这两个节点分别对应**ESVIO**和**ESIO**两种工作模式。

首先分析**ESVIO**工作模式，这是双目+event+IMU进行位姿估计的核心模块。

### 1.stereo_estimator_node的main函数
